...
class App extends React.Component {
  ...
  async openAiSearch(userSearchInput) {
    try {
      ...
      const newUniqueSearchResults = this.removeDuplicateTracks(
        flattenedSearchResults,
      );

      // Combine new results with existing ones
      const combinedSearchResults = [
        ...newUniqueSearchResults,
        ...this.state.searchResults,
      ];
      const finalUniqueSearchResults = this.removeDuplicateTracks(
      ...
      console.log(
        ...
        finalUniqueSearchResults.length,
      );
      // Print the tracks
      ...
      localStorage.setItem(
        "searchResults",
        JSON.stringify(finalUniqueSearchResults),
      );

      this.setState({ isFetching: false });
      if (
        this.state.playlistName === "New Playlist" &&
        this.state.albumArt === defaultAlbumArt
      ) {
  ...
  async generatePlaylistName(prompt) {
    try {
      ...
      this.setState({ playlistName: playlistName }, () => {
        console.log(
          "State updated with playlist name:",
  ...
  clearPlaylist() {
    const confirmClear = window.confirm(
      "Careful! Your playlist will be lost forever unless you save it to Spotify. Are you sure you want to clear the playlist?",
    );
    if (confirmClear) {
      ...
      <div className="App">
        <div className="Header">
          ...
          <div className="user-info">
            <a
              href={`https://open.spotify.com/user/${this.state.spotifyUsername}`}
              target="_blank"
              rel="noopener noreferrer"
            >
              <img
                className="avatar"
                src={this.state.spotifyAvatar || defaultAlbumArt}
                alt="avatar"
              />
            </a>
            <h1 className="username">
              {this.state.spotifyUsername || "User"}{" "}
              <FontAwesomeIcon
                icon={faCopy}
                onClick={() =>
                  navigator.clipboard.writeText(
                    `https://open.spotify.com/user/${this.state.spotifyUsername}`,
                  )
                }
              />
            </h1>
        ...
        <div className="SearchAndPlaylist">
          ...
          >
            <div className="PlaylistSectionHeader">
              ...
              {this.state.playlistName !== "New Playlist" &&
                this.state.albumArt !== "./default-album-art.png" && (
                  <button
